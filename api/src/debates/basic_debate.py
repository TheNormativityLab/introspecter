...
def normalize_model_name(model_name: str) -> str:
    ...
    normalized = RE_NORMALIZE_HYPHEN.sub(r'-\1.\2-', normalized)
    return normalized
    ...
    async def _create_agents_from_models(
        ...
        for llm_config in available_llm_configs:
            ...
            if model_name:
                ...
                if normalized.startswith('vec-'):
                    model_to_config[normalized[4:]] = llm_config
                else:
                    model_to_config[f"vec-{normalized}"] = llm_config
        ...
        for agent_idx, model_name in enumerate(agent_models):
            ...
            if is_human:
                ...
                if not llm_config_dict:
                    raise ValueError(f"Could not find LLM config for model '{model_name}'")
